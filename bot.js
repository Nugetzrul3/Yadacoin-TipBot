...
bot.on('ready', function () {
	if (log1) console.log("YDA TipBot ready and loaded correctly! Hello, admin");
	...
	if (isCallingBot(msg.content) == true) {
		...
		switch (command) {
			...
			case 'adminhelp':
				isAdmin(msg.author.id, function (result) {
					if (result == true) {
						msg.author.send("Hello! Welcome to the ADMIN HELP SECTION. \n To show user info, type \"!tipyada showuserinfo userid\" \n To add an admin, type \"!tipyada addadmin userid\" (only available for owner) \n To remove an admin, type \"!tipyada removeadmin userid\" (only available for owner) \n To disable tipping for a user, type \"!tipyada switchtipsend userid allow/disallow\" \n To disable receive of tips for a user, type \"!tipyada switchtipreceive userid allow/disallow\" \n To stop the bot from listening to commands, type \"!tipyada stoplistening\" \n To start bot listening to commands, type \"!tipyada startlistening\" (only available to bot owner) \n To display wallet info, type \"!tipyada walletinfo\" (only available to bot owner)");
					}
				});
				break;
			case 'help':
				msg.author.send("Hello! Welcome to Yada TipBot help section. \n To get your balance, type \"!tipyada mybalance\" \n For deposits, type \"!tipyada deposit\" \n For withdrawals, type \"!tipyada withdraw <walletaddress> <amount>\" (withdrawal fee is " + withdraw_tx_fees + " " + coin_name + ".), minimum withdrawal amount is " + withdraw_min_amount + " " + coin_name + ". \n To tip someone, type \"!tipyada tip <user_mention> <amount> <Optional: small message>\" \n We are not responsible for any system abuse, please don't deposit/leave big amounts ");
				break;
			case 'mybalance':
